...
"""Deployment script for BigQuery Multi-Agent Application."""
...
from bq_multi_agent_app.agent import root_agent
...
AGENT_WHL_FILE = "bq_multi_agent_app-0.1.0-py3-none-any.whl"
...
def main(argv: list[str]) -> None:  # pylint: disable=unused-argument
    ...
    bucket_name = (
        FLAGS.bucket
        if FLAGS.bucket
        else os.getenv("GCS_STAGING_BUCKET", default_bucket_name)
    )

    # Environment variables for BigQuery Multi-Agent App
    env_vars["TOOLBOX_URL"] = os.getenv("TOOLBOX_URL")
    ...
    if not bucket_name:
        ...
        print(
            "Set the GCS_STAGING_BUCKET environment variable or use --bucket flag."
        )
        return
