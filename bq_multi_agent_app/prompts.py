def return_instructions_root() -> str:
    ...
    **Routing priority (check in order):**
    ...
    ### 1. DATA AGENT PATH
    User explicitly references a pre-configured BQ Data Agent.
    ...
    ### 2. BQML PATH
    Machine learning model operations — delegate to BQML sub-agent.
    ...
    ### 3. ADVANCED PATH
    Deep analysis requiring Python execution or advanced BQ tools — delegate to DS sub-agent.
    ...
    ### 4. DEFAULT PATH
    All other data questions (the vast majority) — use `ask_data_insights` (Conversational Analytics API).
    - Handles: counts, aggregations, trends, comparisons, rankings, simple charts
    - Returns Vega-Lite chart specs rendered natively in Gemini Enterprise — preferred path
      for any visualization request that does not require custom Python
    - This is the right choice for most "show me...", "how many...", "what is..." questions
    - The CA API writes the SQL internally — do not write SQL for questions it can answer
